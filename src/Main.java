...
public class Main {
  ...
  public static String emailLookupEfficient(HashMap<String, String> namesToEmails, String queryName) {
    for (int i = 0; i < namesToEmails.size(); i++) {
      if (namesToEmails.get(i) == queryName) {  
        return namesToEmails.get(i);
      } 
    }
    return "Person not found";
  }
  ...
  // YOUR ANSWER HERE HashSet since location does not matter 
  ...
  // What would be a good choice of data structure?
  // YOUR ANSWER HERE TreeMap since location is important as it's a playlist
  ...
  // What would be a good choice of data structure?
  // YOUR ANSWER HERE TreeMap since order is of most importance
}